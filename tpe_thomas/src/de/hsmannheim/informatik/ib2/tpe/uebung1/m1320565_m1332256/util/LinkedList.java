	...
	public void delete(int index){
		if(index < size()){
			if(index == 0){
				...
				ListNode m = root;
				for(int i = 0; i < (size()-1); i++){ // l will be the element before the element that needs to be removed.
					l = l.next;
				}
				m = l.next.next; // Gets the element one element ahead of the element to remove.
				l = l.next; // Sets m to be the next element after l, deleting the element l+1.
			}
		}
	}
